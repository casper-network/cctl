...
function main()
{
    ...
    GAS_PAYMENT=${GAS_PAYMENT:-$CCTL_DEFAULT_GAS_PAYMENT}
    ...
    CP1_SECRET_KEY=$(get_path_to_secret_key "$CCTL_ACCOUNT_TYPE_FAUCET")
    CP1_ACCOUNT_KEY=$(get_account_key "$CCTL_ACCOUNT_TYPE_FAUCET")
...
main "${AMOUNT:-$CCTL_DEFAULT_TRANSFER_AMOUNT}" \
     "${USER_ID:-1}" \
     "${TRANSFERS:-100}" \
     "${INTERVAL:-0.01}" \
     "${NODE_ID:-"random"}" \
